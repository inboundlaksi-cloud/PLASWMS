...
(function(){
  const PATCH_VERSION = '4.7.0-last-hardfix';
  function ready(fn){
  ...
  function install(){
    const app=window.app; if(!app) return; app._nwV47Patched=true;
    if(!app.state.noteWall) app.state.noteWall={notes:[], mode:'board', viewDate:todayStr(), filterStatus:'all', filterType:'all'};
    ...
    console.log('✅ NOTEWALL_V47_LAST_HARDFIX_ACTIVE');
  }
  ready(function(){
    // ลงซ้ำหลายจังหวะและไม่ใช้ guard เก่า เพื่อให้ทับ renderer ตัวจริงเสมอ
    install();
